...
float isIn = step(sqrt(pow((mousePosU.x - pos.x) * width, 2) + pow((mousePosU.y - pos.y) * height, 2)), 20);

// if the mouse is clicked, draws particles arround the circle
vec4 clicked_circle = isIn * mouseLeftButtonU * (sandModeU * sand_color + wallModeU * wall_color + eraserModeU * black);
